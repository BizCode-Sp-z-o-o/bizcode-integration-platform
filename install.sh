...
x-nodered-common: &nodered-common
  image: bizcode.azurecr.io/integration-platform:latest
  restart: unless-stopped
  environment:
    - TZ=Europe/Warsaw
    - NR_ADMIN_USER=${NR_ADMIN_USER:-admin}
    - NR_ADMIN_PASS=${NR_ADMIN_PASS:-changeme!}
  networks:
    - bip
...
# Generate exactly BIP_COUNT instances
for i in $(seq 0 $((BIP_COUNT - 1))); do
    idx=$(printf '%02d' $i)
    port=$((1880 + i))
...
cat >> docker-compose.yml << 'INFRA'
  ...
  rabbitmq:
    ...
    container_name: bip-rabbitmq
    restart: unless-stopped
    environment:
      - RABBITMQ_DEFAULT_USER=${RABBITMQ_USER:-bizcode}
  ...
  postgres:
    ...
    restart: unless-stopped
    environment:
      - POSTGRES_USER=${POSTGRES_USER:-bizcode}
      - POSTGRES_PASSWORD=${POSTGRES_PASS:-changeme}
      - POSTGRES_DB=${POSTGRES_DB:-bizcode}
    ports:
      - "5432:5432"
    volumes:
  ...
  cups:
    image: ydkn/cups
    container_name: bip-cups
    restart: unless-stopped
    environment:
      - ADMIN_PASSWORD=${CUPS_ADMIN_PASS:-changeme}
    ports:
      - "631:631"
    volumes:
      - cups-data:/etc/cups
...
for i in $(seq 0 $((BIP_COUNT - 1))); do
    idx=$(printf '%02d' $i)
    echo "  bip-${idx}-data:" >> docker-compose.yml
done
cat >> docker-compose.yml << 'VOLS'
  redis-data:
  rabbitmq-data:
  postgres-data:
  cups-data:
...
info "docker-compose.yml generated (${BIP_COUNT} instances)"

# ── Generate docker-compose.prod.yml (prod mode) ──
if [ "$MODE" = "prod" ]; then
    header "Generating docker-compose.prod.yml..."

    cat > docker-compose.prod.yml << 'PROD_HEADER'
# Auto-generated by install.sh — production overlay
# Removes direct port exposure, adds Nginx Proxy Manager
services:
PROD_HEADER

    # Remove ports from all instances
    for i in $(seq 0 $((BIP_COUNT - 1))); do
        idx=$(printf '%02d' $i)
...
EOF
    ...
    cat >> docker-compose.prod.yml << 'PROD_NPM'

  # ── Nginx Proxy Manager ──
  npm:
    image: jc21/nginx-proxy-manager:latest
    container_name: bip-npm
    restart: unless-stopped
    ports:
      - "80:80"
      - "443:443"
...
PROD_NPM
    ...
    info "docker-compose.prod.yml generated"
fi
